            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
                ...
                {result.fetched_at && (
                  <span className="text-xs text-gray-500">
                    Updated {new Date(result.fetched_at).toLocaleString('vi-VN')}
                  </span>
                )}
              ...
              {(result.earliest_harvest_date || result.latest_harvest_date) && (
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-3">
                    <p className="text-xs font-semibold text-emerald-700">Earliest</p>
                    <p className="mt-1 font-bold text-gray-900">{new Date(result.earliest_harvest_date).toLocaleDateString('vi-VN')}</p>
                  </div>
                  <div className="rounded-lg border border-blue-100 bg-blue-50 p-3">
                    <p className="text-xs font-semibold text-blue-700">Optimal</p>
                  ...
                  <div className="rounded-lg border border-amber-100 bg-amber-50 p-3">
                    <p className="text-xs font-semibold text-amber-700">Latest</p>
              ...
              {(result.weather_risk || result.market_condition) && (
                <div className="rounded-lg border border-gray-200 p-4">
                  <p className="text-sm font-semibold text-gray-700">AI risk factors</p>
                  <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <span className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">Weather: {result.weather_risk}</span>
                    <span className="rounded-md bg-green-50 px-3 py-2 text-sm text-green-700">Market: {result.market_condition}</span>
                  </div>
                </div>
              )}
              {result.preparation_tasks?.length > 0 && (
                <div className="rounded-lg border border-gray-200 p-4">
                  <p className="text-sm font-semibold text-gray-700">Preparation checklist</p>

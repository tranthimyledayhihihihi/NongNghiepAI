import { ArrowRight, Clock, Cloud, Droplets, MapPin, Sparkles, TrendingUp, Wind } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      dashboardApi.getSummary(region, { forceRefresh: true }),
      ...
      .then(([summary, trend, recommendation]) => {
        ...
        if (weatherData) {
          setWeatherLive({
            ...weatherData,
            temp: Number(weatherData.temperature ?? 28).toFixed(1),
            humidity: `${Math.round(weatherData.humidity ?? 0)}%`,
            rainfall: `${Number(weatherData.rainfall ?? 0).toFixed(1)} mm`,
            wind: `${Number(weatherData.wind_speed ?? 0).toFixed(1)} km/h`,
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Thời tiết hiện tại</h3>
            ...
            <div className="text-center mb-4">
              ...
              {displayedWeather.checked_at && (
                <p className="mt-2 inline-flex items-center gap-1 text-xs text-gray-500">
                  <Clock className="h-3.5 w-3.5" />
                  {new Date(displayedWeather.checked_at).toLocaleString('vi-VN')}
                </p>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 bg-amber-50 rounded-lg">
                <p className="text-sm text-amber-800">Mưa hiện tại: {displayedWeather.rainfall}</p>
              </div>

              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                <div className="flex items-center">
                  <Wind className="w-5 h-5 text-slate-600 mr-2" />
                  <span className="text-sm text-gray-700">Gió</span>
                </div>
                <span className="text-sm font-semibold text-gray-900">{displayedWeather.wind}</span>
              </div>
            </div>
          </div>
